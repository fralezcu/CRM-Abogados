...
module.exports = class ReportsDBApi {
  static async create(data, options) {
    ...
    await reports.setLeads(data.leads || [], {
      transaction,
    });

    await reports.setSales(data.sales || [], {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    output.leads = await reports.getLeads({
      transaction,
    });

    output.sales = await reports.getSales({
      transaction,
    });
  ...
  static async findAll(filter, options) {
    ...
    let include = [
...

      {
        model: db.leads,
        as: 'sales',
        through: filter.sales
          ? {
              where: {
                [Op.or]: filter.sales.split('|').map((item) => {
                  return { ['Id']: Utils.uuid(item) };
                }),
              },
            }
          : null,
        required: filter.sales ? true : null,
      },
    ];
